...
(function ($) {
  ...
  $.fn.zoom = function (options) {
    ...
    var mouseMoveHandler = function (e) {
      ...
      if($mouseZoom.css("visibility") !== "hidden") {
        e.stopPropagation();
      }
      ...
      var top = y - ($mouseZoom.height() / 2);
      if (top < 0) { top = 0; }
      if ((top + $mouseZoom.height()) > $(this).height()) { top = $(this).height() - $mouseZoom.height(); }

      var left = x - ($mouseZoom.width() / 2);
      if (left < 0) { left = 0; }
      if (left + $mouseZoom.width() > $(this).width()) { left = $(this).width() - $mouseZoom.width(); }
    ...
    var getOptions = function ($container) {
      return $container.data(optionsKey);
    }
    ...
    if (options === 'reset') {
      ...
      this.append($('<div class="jsZoomMouse" style="visibiliy: none; position: absolute; z-index: 9999; background-repeat: no-repeat;"/>'));
